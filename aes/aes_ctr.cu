...
namespace aes {
	namespace ctr {
		...
		__global__ void kern_aes_encrypt_ctr(uint8_t *buf_d, uint8_t *key_d, IV* iv_d, int numbytes, int rounds) {
			...
			// TODO consider moving key to local or shared memory also
...
#pragma unroll // todo make a perforamnce analysis on this. could be cool. just macro it 
		...
		int aes::ctr::aes_ctr_encrypt(aes_info* aes)
		{
			uint8_t* buf_d;
			uint8_t* key_d;
			IV* iv_d;
			...
			cudaMalloc((void**)&iv_d, sizeof(IV));
		...
		// byte level parallelization. Every thread gets one byte into the 128 bit block
		int aes::ctr::aes_ctr_decrypt(aes_info* aes)
		{
			uint8_t* buf_d;
			uint8_t* key_d;
			IV* iv_d;
