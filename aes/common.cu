...
namespace aes {
	namespace Common {
		...
		aes_info* aes::Common::create_aes_struct(std::string File,int type)
		{
			...
			int padded_length = length + (16 - (length % 16)); // pad
			...
			uint8_t* keys = new uint8_t[key_words];
			...
			for (int i = 0; i < key_words; i++) keys[i] = i;
		...
		void aes::Common::KeyExpansion(uint8_t* RoundKey, const uint8_t* Key)
		{
			unsigned i, j, k;
